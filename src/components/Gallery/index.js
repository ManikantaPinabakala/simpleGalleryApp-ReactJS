...
class Gallery extends Component {
  state = {activeThumbnailItem: imagesList[0]}

  onChangeThumbnail = id => {
    const updateActiveItem = imagesList.filter(each => each.id === id)
    this.setState({activeThumbnailItem: updateActiveItem[0]})
  }

  render() {
    const {activeThumbnailItem} = this.state
    const {imageUrl, imageAltText} = activeThumbnailItem
        ...
        <ul className="thumbnail-list">
          {imagesList.map(eachThumbnail => (
            <ThumbnailItem
              ...
              isActive={activeThumbnailItem.id === eachThumbnail.id}
            />
          ))}
        </ul>
      </div>
